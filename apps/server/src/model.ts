...
memories: at most 20 atomic, reusable facts. Each item is {kind, content, confidence, moduleIds, evidence, action?, targetId?}; kind is fact, decision, preference, pattern, or issue. Only return a memory when confidence is exactly "high": it must be directly and unambiguously supported by the supplied evidence, not inferred from a plan or a single ambiguous statement. High confidence is about evidence quality, not durability: never record a transient or one-off failure (a single failed command, a flaky test, a momentary network, provider, or tool error) as a memory — it will not hold in future sessions; recurring failure behavior belongs in findings (model_error or tool_misuse), not memories. A memory must be knowledge expected to stay true in future sessions; state an environment constraint (a missing tool, a command that never works here) only when the evidence shows it consistently, never from a single failure. moduleIds contains at most 3 supplied structureTree node ids that the memory directly concerns; use the most specific nodes and [] when no supplied node applies. Evidence items cite only supplied sessionId and optional turnPosition.
Consolidate against knownMemories (each carries its id): when the evidence restates or re-confirms a known memory, return that memory with action "reinforce" and targetId set to its id (content may be the same or a cleaner merge); when the evidence corrects or refines an outdated known memory, return the updated statement with action "supersede" and targetId set to its id. Reference each targetId at most once per inspection. Durable dependencies (this project's resources depending on another project, k8s manifests depending on a CRD, module boundaries, build/runtime prerequisites) are exactly the kind of recurring fact that must be reinforced, not re-created with different wording. Omit action/targetId only for genuinely new knowledge; never repeat a known memory verbatim as new. knownMemories are consolidation references only: reinforce or supersede a memory only when the messages in THIS inspection provide fresh supporting evidence — a memory merely appearing in knownMemories, or being restated by earlier context, is not evidence by itself.
